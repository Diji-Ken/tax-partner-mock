          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${rankColors[client.rank]}`}>
                      {client.rank}
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${softwareColors[client.accountingSoftware]}`}>
                  ...
                  <div className="border-l-4 border-[#ea580c] bg-[#fff7ed] rounded-r-lg p-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-sm text-slate-600">{'IT\u5C0E\u5165\u88DC\u52A9\u91D1\u306E\u5BFE\u8C61\u3068\u306A\u308B\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059\u3002\u696D\u7A2E\u30FB\u898F\u6A21\u304B\u3089\u6700\u5927450\u4E07\u5186\u306E\u88DC\u52A9\u304C\u53D7\u3051\u3089\u308C\u307E\u3059\u3002'}</p>
                      </div>
                      <div className="flex items-start gap-2">
                        <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" />
                        <p className="text-sm text-slate-600">{'\u6C7A\u7B97\u6708\u304C\u8FD1\u3065\u3044\u3066\u3044\u307E\u3059\u3002\u7BC0\u7A0E\u5BFE\u7B56\uFF08\u5C0F\u898F\u6A21\u4F01\u696D\u5171\u6E08\u30FB\u7D4C\u55B6\u30BB\u30FC\u30D5\u30C6\u30A3\u5171\u6E08\uFF09\u306E\u63D0\u6848\u3092\u63A8\u5968\u3057\u307E\u3059\u3002'}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                      ...
                      {monthlySteps.map((step, i) => (
                        <React.Fragment key={i}>
                          <div className="flex flex-col items-center gap-1.5">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${step.done ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
                              {step.done ? '\u2713' : i + 1}
                            </div>
                            <span className="text-[10px] text-slate-500 text-center whitespace-nowrap">{step.label}</span>
                          </div>
                          {i < monthlySteps.length - 1 && (
                            <div className={`flex-1 h-0.5 ${step.done ? 'bg-emerald-400' : 'bg-slate-200'} mt-[-18px]`} />
                          )}
                        </React.Fragment>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    {clientTasks.length > 0 ? (
                      ...
                      <div className="text-center py-8 text-slate-400">
                        ...
                        <p className="text-sm">{'\u30BF\u30B9\u30AF\u306F\u307E\u3060\u3042\u308A\u307E\u305B\u3093'}</p>
                        <button className="mt-2 text-xs text-[#ea580c] border border-[#ea580c] rounded px-3 py-1.5 hover:bg-[#ea580c] hover:text-white transition-colors">+ {'\u30BF\u30B9\u30AF\u3092\u8FFD\u52A0'}</button>
                      </div>
                    )}
                  </div>
                  {/* Task templates */}
                  {dbTaskTemplates.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {dbTaskTemplates.slice(0, 5).map((tmpl) => (
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-slate-200 rounded-lg p-6 text-center mb-4 hover:border-[#ea580c] transition-colors cursor-pointer">
                      ...
                      <div className="flex justify-center gap-2 mt-2">
                        <button className="text-xs text-slate-500 border border-slate-200 rounded px-3 py-1 hover:bg-slate-50">{'Google Drive\u304B\u3089\u9078\u629E'}</button>
                        <button className="text-xs text-slate-500 border border-slate-200 rounded px-3 py-1 hover:bg-slate-50">{'\u30B9\u30AD\u30E3\u30CA\u30FC\u9023\u643A'}</button>
                      </div>
                    </div>
                    {clientOcr.length > 0 && (
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                          <thead><tr className="border-b border-slate-100">{['\u30B9\u30C6\u30FC\u30BF\u30B9', '\u8A3C\u6191\u7A2E\u5225', '\u53D6\u5F15\u5148', '\u91D1\u984D', '\u52D8\u5B9A\u79D1\u76EE', '\u64CD\u4F5C'].map((h) => (<th key={h} className="text-left py-2 text-slate-400 font-medium px-2">{h}</th>))}</tr></thead>
                          <tbody>
                            {clientOcr.map((ocr) => (
                              <tr key={ocr.id} className="border-b border-slate-50">
                                <td className="py-2 px-2"><span className={`text-[10px] font-medium px-2 py-0.5 rounded ${ocr.status === '\u78BA\u8A8D\u6E08' ? 'bg-emerald-100 text-emerald-700' : ocr.status === '\u8981\u78BA\u8A8D' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>{ocr.status}</span></td>
                                ...
                                <td className="py-2 px-2"><button className="text-[#ea580c] hover:underline">{'\u78BA\u8A8D'}</button></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        ...
                        {[{ name: '2025\u5E74\u5EA6', items: ['\u9818\u53CE\u66F8', '\u8ACB\u6C42\u66F8', '\u9280\u884C\u660E\u7D30', '\u30AF\u30EC\u30AB\u660E\u7D30', '\u6C7A\u7B97\u66F8\u985E'] }, { name: '2026\u5E74\u5EA6', items: ['\u9818\u53CE\u66F8', '\u8ACB\u6C42\u66F8', '\u9280\u884C\u660E\u7D30', '\u30AF\u30EC\u30AB\u660E\u7D30'] }].map((folder) => (
                          <div key={folder.name} className="bg-slate-50 rounded-lg p-3">
                            ...
                            <div className="flex flex-wrap gap-1.5 ml-6">
                              {folder.items.map((item) => (<span key={item} className="text-xs bg-white border border-slate-200 rounded px-2 py-1 text-slate-600">{item}</span>))}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="space-y-2">
                      {clientInvoicesDemo.map((inv) => (
                        <div key={inv.id} className="flex items-center gap-4 py-3 px-4 bg-slate-50 rounded-lg hover:bg-slate-100 cursor-pointer">
                          ...
                          <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${inv.status === '\u5165\u91D1\u6E08' ? 'bg-emerald-100 text-emerald-700' : inv.status === '\u9001\u4ED8\u6E08' ? 'bg-blue-100 text-blue-700' : inv.status === '\u672A\u5165\u91D1' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>{inv.status}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
